...
import {
  FALLBACK_SURAHS, fetchSurah, fetchSurahList, searchQuran, RECITERS, TRANSLATIONS,
  type SearchHit, type SurahDetail, type SurahMeta,
} from "@/lib/quran";
import { GaplessQuranPlayer } from "@/lib/quran-gapless";
...
export default function QuranExperience() {
  ...
  const playerRef = useRef<GaplessQuranPlayer | null>(null);
  ...
  useEffect(() => {
    const player = new GaplessQuranPlayer({
      onAyah: (ref) => {
        playTargetRef.current = ref;
        setCurrentAyah(ref.ayah);
        saveStore({ surah: ref.surah, ayah: ref.ayah });
        setProgress(ref);
        if (ref.surah !== surahNumRef.current) {
          router.replace(`/quran?surah=${ref.surah}&ayah=${ref.ayah}`, { scroll: false });
        }
      },
      onPlay: () => {
  ...
  const playAyah = useCallback((surah: number, ayah: number) => {
    ...
    playTargetRef.current = { surah, ayah: safeAyah };
    setCurrentAyah(safeAyah);
    saveStore({ surah, ayah: safeAyah });
    setProgress({ surah, ayah: safeAyah });
    const player = playerRef.current;
    if (!player) return;
    player.setReciter(reciterRef.current);
    player.setRepeat(repeat);
    player.setSpeed(speed);
    player.setVolume(volume);
    void player.play({ surah, ayah: safeAyah });
  }, [repeat, speed, volume]);
  ...
  useEffect(() => {
    playerRef.current?.setRepeat(repeat);
  }, [repeat]);
